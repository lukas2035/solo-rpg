...
export default function SidePanel({ tab, onTabChange, counts, onClose, children }: SidePanelProps) {
  return (
    <aside className="w-[400px] max-w-full flex-shrink-0 h-full flex flex-col bg-[#0c0c0c]/95 border-l-2 border-[var(--accent)]/40 text-[var(--text)]">
      <header className="flex items-stretch gap-1 px-2 pt-2 border-b border-[var(--accent)]/30">
        {TABS.map(t => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-t-lg border border-b-0 text-sm font-semibold transition-colors ${
                selected
        ...
        <button type="button" onClick={onClose} title="Zavřít panel" className="ml-auto self-center w-8 h-8 rounded-lg border border-[var(--accent)]/40 hover:border-[var(--accent)] transition-colors">✕</button>
      </header>
      {children}
    </aside>
  )
